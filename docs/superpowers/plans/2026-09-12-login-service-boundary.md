...
- [ ] Characterize the actual HTTP route before changing it. Extend the existing
...
- [ ] Write new direct service tests red against the missing module/service.
...
- [ ] Add exception regressions: user-state read or verifier raises after a
...
- [ ] Implement the decision with the same branch order and checks as `_login`.
...
- [ ] Adapt `_login` with the current trusted client-IP and configured host
...
- [ ] Register `login_service.py` beside `login_throttle.py` in durable artifacts,
...
- [ ] Run new direct tests, complete reliability regressions, auth routes/views,
...
- [ ] Self-review and commit only owned source/tests locally; report exact
