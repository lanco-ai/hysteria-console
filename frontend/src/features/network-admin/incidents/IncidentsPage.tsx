...
function Content({ data, retry }: { data: Incidents; retry: () => void }) {
  ...
  return <div className="admin-page incidents-page">
    ...
    <div className="health-top-kpis"><div className="health-kpi-card"><div className="health-kpi-label">峰值小时</div><div className="health-kpi-value mono">{fmtBytes(data.peak_hour.bytes)}</div><div className="health-kpi-sub">{data.peak_hour.hour || '—'}</div></div><div className="health-kpi-card"><div className="health-kpi-label">当前小时</div><div className="health-kpi-value mono">{fmtBytes(data.stats.current_hour_bytes)}</div><div className="health-kpi-sub">{data.stats.online} 在线</div></div><div className="health-kpi-card"><div className="health-kpi-label">受影响用户</div><div className="health-kpi-value mono">{affected}</div><div className="health-kpi-sub">峰值小时内</div></div><div className="health-kpi-card"><div className="health-kpi-label">活跃告警</div><div className="health-kpi-value mono">{data.alerts.length}</div><div className="health-kpi-sub">当前状态</div></div></div>
    ...
    <section className="admin-section"><div className="admin-section-header"><div><h2 className="admin-section-title">处置候选用户</h2><div className="small">按近 24 小时流量排序</div></div></div><div className="admin-section-body no-pad"><div className="data-table-wrap" tabIndex={0} aria-label="处置候选用户，可横向滚动"><table className="data-table"><thead><tr><th>用户</th><th>24h 流量</th><th>周期用量</th><th>状态</th><th>操作</th></tr></thead><tbody>{data.users.length ? data.users.map(row => <tr key={row.user}><th scope="row"><div>{row.user}</div><div className="small faint">{row.note || row.expiry_label}</div></th><td className="mono">{fmtBytes(row.last_24h_bytes)}</td><td className="mono">{fmtBytes(row.cycle_used_bytes)} · {row.quota_bytes ? `${row.quota_percent.toFixed(1)}%` : '不限额'}</td><td>{row.disabled || row.expired ? <Status ok={false} label={row.disabled ? '已停用' : '已过期'}/> : <Status ok label={`${row.online} 在线`}/>}</td><td><div className="row gap-sm"><ActionButton action="pause-user" row={row} disabled={false} done={setMessage}/><ActionButton action="rotate-token" row={row} disabled={false} done={setMessage}/><a className="btn ghost btn-sm" href={`/admin/user/${encodeURIComponent(row.user)}`}>画像</a></div></td></tr>) : <tr><td className="empty" colSpan={5}>暂无用户</td></tr>}</tbody></table></div></div></section>
