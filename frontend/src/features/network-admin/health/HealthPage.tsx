...
export function HealthPage({ publicHost }: { publicHost: string }) {
  ...
  return <AdminShell active="health" pageTitle="健康状态" badge={health.status === 'success' ? '实时探测' : ''} subtitle={`${publicHost} · 服务与基础设施`} topbarExtra={<><button className="btn ghost btn-sm" type="button" onClick={refresh} disabled={health.status === 'loading'}>立即刷新</button><button className="btn ghost btn-sm" type="button" onClick={() => void runAction('update-check')} disabled={Boolean(actionBusy)}>检查更新</button><button className="btn secondary btn-sm" type="button" onClick={() => void runAction('update-apply', {}, '将更新任务加入后台队列，确认继续？')} disabled={Boolean(actionBusy)}>立即更新</button><button className="btn ghost btn-sm" type="button" onClick={() => void runAction('test-alert')} disabled={Boolean(actionBusy)}>测试告警</button><span className="badge poll-status">{polling}</span><span className="sr-only" role="status" aria-live="polite">{polling}</span></>}>
