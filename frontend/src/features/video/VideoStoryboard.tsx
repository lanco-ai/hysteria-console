...
export function VideoStoryboard({ storyboard, capabilities, saveState, busy, onChange, onSave, onRunShot, onUploadImage, onRunAll, onOpenCanvas }: VideoStoryboardProps): ReactElement {
  ...
  return <section className="video-storyboard" aria-label="AI 漫剧分镜工作台">
    <header className="video-storyboard-header">
      ...
      <div className="video-storyboard-header-actions"><label className="video-ratio-field">画幅<select value={storyboard.aspect_ratio} onChange={event => updateProject({ aspect_ratio: event.target.value as VideoStoryboard['aspect_ratio'] })}><option value="9:16">竖屏 9:16</option><option value="16:9">横屏 16:9</option><option value="1:1">方形 1:1</option></select></label><button type="button" className="btn btn-ghost" onClick={onOpenCanvas}>展开画布</button><button type="button" className="btn btn-secondary" onClick={onSave} disabled={busy}>保存</button><button type="button" className="btn btn-primary" onClick={onRunAll} disabled={busy || !storyboard.shots.length}>运行全部</button></div>
    </header>
    <div className="video-storyboard-body">
      <aside className="video-script-panel">
        <div className="video-panel-heading"><div><span className="video-eyebrow">故事文本</span><h2>文案改写</h2></div><button type="button" className="btn btn-ghost btn-sm" onClick={() => updateProject({ rewritten_text: storyboard.source_text })} disabled={!storyboard.source_text.trim()}>改写</button></div>
        ...
        <button type="button" className="btn btn-secondary video-split-button" onClick={() => updateProject({ shots: storyboard.shots.length ? storyboard.shots : [createStoryboardShot(0)] })}>按分镜整理</button>
      </aside>
      <main className="video-shot-list">
        ...
        {storyboard.shots.map((shot, index) => <article className="video-storyboard-row" data-shot-id={shot.id} key={shot.id}>
          ...
          <div className="video-shot-row-grid">
            ...
            <section className="video-shot-card video-shot-motion"><div className="video-card-heading"><span>运镜</span><span>{firstLastSupported ? '首尾帧可用' : '普通图生视频'}</span></div><label>视频模型<select value={shot.video_model} onChange={event => field(storyboard, shot.id, 'video_model', event.target.value, onChange)}>{[shot.video_model, ...(capabilities?.video_models || [])].filter((model, index, models) => model && models.indexOf(model) === index).map(model => <option key={model} value={model}>{model}</option>)}</select></label><label>运动描述<textarea value={shot.motion_prompt} onChange={event => field(storyboard, shot.id, 'motion_prompt', event.target.value, onChange)} placeholder="镜头缓慢推进，人物自然转身…" rows={4} /></label><label>对白 / 旁白<textarea value={shot.dialogue} onChange={event => field(storyboard, shot.id, 'dialogue', event.target.value, onChange)} placeholder="可选" rows={2} /></label><button type="button" className="btn btn-primary btn-sm" onClick={() => onRunShot(shot)} disabled={busy || (!shot.image_prompt.trim() && !shot.image_asset_id)}>运行当前镜头</button></section>
    ...
    <footer className="video-storyboard-footer"><span>素材生成由当前配置的第三方 API 完成</span><span>{storyboard.style_prompt ? '已设置统一风格' : '建议先设置统一风格'}</span><button type="button" className="btn btn-ghost btn-sm" disabled>片段合成（待供应商支持）</button></footer>
  </section>;
}
