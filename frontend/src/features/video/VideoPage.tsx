...
export function VideoPage({ publicHost }: { publicHost: string }): ReactElement {
  ...
  const save = async (): Promise<string | null> => {
    ...
    try {
      ...
      setWorkflowId(saved.id); setWorkflows(current => [saved, ...current.filter(item => item.id !== saved.id)]); setMessage('工作流已保存'); return saved.id;
    } catch (error) { setMessage(error instanceof Error ? error.message : '保存失败'); return null; } finally { setBusy(false); }
  };
  const run = async () => {
    const id = workflowId || await save();
    if (!id) return;
    setBusy(true);
    try { const result = await createRun(id); setRunId(result.id); setMessage(`任务 ${result.state}`); } catch (error) { setMessage(error instanceof Error ? error.message : '提交失败'); } finally { setBusy(false); }
  };
  ...
  return <CodexShell active="video" pageTitle="AI 视频" authStatus="authenticated" subtitle="工作流">
    <section className="video-workspace">
      <header className="video-toolbar"><div><h2>AI 视频工作流</h2><p>{message}</p></div><div className="video-toolbar-actions"><button type="button" className="button ghost" onClick={() => setSettingsOpen(true)}>API 设置</button><button type="button" className="button secondary" onClick={() => { void save(); }} disabled={busy}>保存</button><button type="button" className="button primary" onClick={() => { void run(); }} disabled={busy}>运行工作流</button></div></header>
      ...
      <VideoRunPanel runId={runId} />
      <footer className="video-run-status"><span>编辑画布不会自动产生费用</span></footer>
    </section>
