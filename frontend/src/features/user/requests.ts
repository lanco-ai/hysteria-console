import { readAccessCode, ResourceError } from '../../shared/readResource';
...
function unlimited(value: unknown): number {
  const result = integer(value);
...
export async function readUserPanel(signal: AbortSignal): Promise<UserPanel> {
  ...
  if (!response.ok) throw new ResourceError(`HTTP ${response.status}`, response.status, await readAccessCode(response));
