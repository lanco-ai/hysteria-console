  ...
  return <>
    ...
    {open ? <aside className="lanco-agent" role="dialog" aria-modal="false" aria-labelledby="lanco-agent-title">
      ...
      <div className="lanco-agent-body">
        ...
        {result ? <section className="lanco-agent-result"><p>{result.explanation}</p>{result.snapshot ? <p className="small">当前有 {result.snapshot.rules.length} 条用户规则，版本 {result.snapshot.revision.slice(0, 8)}…</p> : null}{result.plan ? <><div className="lanco-agent-plan"><strong>{result.plan.label}</strong><span>{result.plan.description}</span>{result.plan.additions.length ? <ul>{result.plan.additions.map(rule => <li key={rule}><code>{rule}</code></li>)}</ul> : <span>没有新增规则，重复项已跳过。</span>}</div><div className="lanco-agent-actions"><button className="btn btn-primary btn-sm" type="button" onClick={() => void apply()} disabled={status !== 'preview'}>应用修改</button><button className="btn btn-ghost btn-sm" type="button" onClick={() => { setResult(null); setLastChangeId(''); setStatus('idle'); }}>取消</button></div></> : null}{result.action === 'inspect' && status === 'saved' ? <button className="btn btn-ghost btn-sm" type="button" onClick={() => void undo()} disabled>暂无可撤销变更</button> : null}{result.plan && status === 'saved' ? <button className="btn btn-ghost btn-sm" type="button" onClick={() => void undo()}>撤销这次修改</button> : null}</section> : null}
