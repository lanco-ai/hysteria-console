...
export function LancoAgent() {
  ...
  const resetPosition = (target: DragTarget) => {
    if (target === 'panel') setPanelPosition(null);
    else setLauncherPosition(null);
  };
  ...
  return <>
    ...
    {open ? <aside ref={panelRef} className={`lanco-agent${draggingTarget === 'panel' ? ' is-dragging' : ''}`} style={panelStyle} role="dialog" aria-modal="false" aria-labelledby="lanco-agent-title">
      ...
      <div className="lanco-agent-body">
        ...
        {!isIdle ? <div className="lanco-agent-timeline" aria-live="polite"><span className={status !== 'idle' ? 'is-active' : ''}>读取用户规则</span><span className={status === 'read' || status === 'preview' || status === 'applying' || status === 'saved' || status === 'undone' ? 'is-active' : ''}>检查冲突并生成预览</span><span className={status === 'saved' || status === 'undone' ? 'is-active' : ''}>{status === 'undone' ? '已撤销修改' : '保存结果'}</span></div> : null}
