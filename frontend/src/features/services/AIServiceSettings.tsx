...
const featureGroups: { title: string; description: string; features: Feature[] }[] = [
  { title: '对话与助手', description: '为日常对话和智能建议选择服务。', features: ['chat', 'plan_assistant', 'video_assistant'] },
  { title: '媒体生成', description: '为图片和视频任务选择生成服务。', features: ['image_generation', 'video_generation'] },
];
...
export function AIServiceSettings() {
  ...
  return <section className="ai-service-settings" aria-label="API 服务接入">
    ...
    {catalog ? <>
      <div className="ai-service-section-heading"><span className="services-eyebrow">接入服务</span><h3>已连接的接口</h3><p>查看连接状态，按需编辑服务配置。</p></div>
      <div className="ai-service-grid">
        {catalog.profiles.map(profile => {
          ...
          return <article className="ai-service-card" data-ai-service={profile.id} key={profile.id}>
            ...
            <div className="ai-service-card-summary"><strong>{profile.models.length} 个模型</strong><span>{profile.last_verified_at ? `最近验证 ${new Date(profile.last_verified_at).toLocaleString()}` : '尚未验证连接'}</span></div>
            <div className="ai-service-card-actions"><button type="button" className="btn service-secondary" disabled={busy !== '' || !profile.api_key_configured || hasUnsavedChanges} onClick={() => void refreshModels(profile)}>{busy === `test:${profile.id}` ? '连接中…' : '测试连接'}</button></div>
      ...
      <section className="ai-service-catalog" aria-labelledby="ai-service-catalog-title">
        <header className="ai-service-catalog-heading"><div><span className="services-eyebrow">模型目录</span><h2 id="ai-service-catalog-title">可用模型</h2><p>选择服务并搜索已验证的模型列表。</p></div><span className="ai-service-catalog-count">{modelProfile?.models.length ?? 0} 个模型</span></header>
        <div className="ai-service-catalog-controls"><label>服务<select aria-label="模型目录服务" value={modelProfile?.id ?? ''} onChange={event => { setModelProfileId(event.target.value); setModelSearch(''); }}>{catalog.profiles.map(profile => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><label>搜索模型<input type="search" aria-label="搜索模型" value={modelSearch} onChange={event => setModelSearch(event.target.value)} placeholder="输入模型名称或 ID" /></label></div>
        {modelProfile?.last_verified_at ? <p className="ai-service-catalog-verified">最近验证：{new Date(modelProfile.last_verified_at).toLocaleString()}</p> : null}
        {visibleModels.length ? <ul className="ai-service-catalog-list">{visibleModels.map(model => <li key={model.id}><div><strong>{model.name || model.id}</strong>{model.name && model.name !== model.id ? <code>{model.id}</code> : null}</div><div className="ai-service-catalog-meta">{model.context_window != null ? <span>上下文上限 {model.context_window.toLocaleString()} tokens</span> : null}{model.input_token_limit != null ? <span>输入上限 {model.input_token_limit.toLocaleString()} tokens</span> : null}</div></li>)}</ul> : <p className="ai-service-catalog-empty">{modelProfile?.models.length ? '没有匹配的模型。' : '尚未读取模型列表。请先测试对应服务的连接。'}</p>}
      </section>
      <section className="ai-service-bindings" aria-labelledby="ai-service-bindings-title"><header><div><span className="services-eyebrow">功能路由</span><h2 id="ai-service-bindings-title">服务绑定</h2><p>为各项功能选择服务和模型。</p></div></header><div className="ai-service-binding-groups">
        {featureGroups.map(group => <section className="ai-service-binding-group" key={group.title}><header><h3>{group.title}</h3><p>{group.description}</p></header><div className="ai-service-binding-list">
        {group.features.map(featureId => {
          const feature = featureLabels.find(item => item.id === featureId)!;
          ...
          return <div className="ai-service-binding" data-ai-binding={feature.id} key={feature.id}>
            <div className="ai-service-binding-heading"><strong>{feature.label}</strong>{bindingStatus[feature.id] ? <span role="status">{bindingStatus[feature.id]}</span> : null}</div>
            <div className={`ai-service-binding-controls${assistant ? '' : ' is-simple'}`}><label>服务<select aria-label={`${feature.label} 服务`} value={profileId} disabled={busy !== ''} onChange={event => { setBindingDrafts(current => ({ ...current, [feature.id]: event.target.value })); if (assistant) setModelDrafts(current => ({ ...current, [feature.id as AssistantFeature]: '' })); setBindingStatus(current => ({ ...current, [feature.id]: '' })); }}>{compatible.map(profile => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label>
            {assistant && assistantFeature ? <>
              <label>模型<select aria-label={`${feature.label} 模型`} value={modelId} disabled={busy !== '' || !selectedProfile} onChange={event => { setModelDrafts(current => ({ ...current, [assistantFeature]: event.target.value })); setBindingStatus(current => ({ ...current, [assistantFeature]: '' })); }}>
            ...
            <button type="button" className="btn service-secondary" disabled={busy !== '' || !compatible.length} onClick={() => void saveBinding(feature.id)}>保存绑定</button></div>
            {assistant && assistantFeature && selectedProfile ? <details className="ai-service-assistant-tests"><summary>能力测试</summary><div className="ai-service-assistant-test-content"><p>B/C 会向上游发送真实生成请求，但不会保存计划或视频项目。</p>
              ...
              {activeProfileTest && activeProfileTest.revision === catalog.revision && !profileDirty ? <span role="status">A 模型列表读取通过 · {activeProfileTest.models_count} 个 · 配置版本 {activeProfileTest.revision} · {new Date(activeProfileTest.tested_at).toLocaleString()}</span> : null}
