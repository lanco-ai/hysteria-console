...
function storedPreference(name: string, expected: string): boolean {
  try { return localStorage.getItem(name) === expected; } catch { return false; }
}

export function applyInitialShellPreferences(): void {
  if (storedPreference('hy2.sidebar', 'collapsed')) document.documentElement.classList.add('sidebar-pre-collapsed');
...
export function CodexShell({ active, pageTitle, badge, children, subtitle, topbarExtra, sidebarTop, sidebarBottom, authStatus, agentEnabled = false }: CodexShellProps) {
  ...
  const navigation = navigationGroups.filter(group => group.placement !== 'bottom');
  const footerNavigation = navigationGroups.flatMap(group => group.placement === 'bottom' ? group.items : []);
  ...
  useLayoutEffect(() => {
    if (!open && restoreFocusRef.current) { restoreFocusRef.current = false; toggleRef.current?.focus(); }
  }, [open]);
  useLayoutEffect(() => {
    if (mobile && open) sidebarRef.current?.querySelector<HTMLElement>('#sidebar-close')?.focus();
  }, [mobile, open]);
  useLayoutEffect(() => {
    if (logout.failureCount === 0 || handledLogoutFailureRef.current === logout.failureCount) return;
    if (mobile && open) { restoreFocusRef.current = false; setOpen(false); return; }
  ...
  useEffect(() => { document.body.classList.toggle('sidebar-open', mobile && open); }, [mobile, open]);
  useEffect(() => {
    const onResize = () => { const nextMobile = window.innerWidth <= MOBILE_BREAKPOINT; setMobile(nextMobile); if (!nextMobile) setOpen(false); };
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') { event.preventDefault(); restoreFocusRef.current = true; setOpen(false); return; }
      ...
      if (event.shiftKey && (activeElement === first || !sidebarRef.current.contains(activeElement))) { event.preventDefault(); last.focus(); }
      else if (!event.shiftKey && (activeElement === last || !sidebarRef.current.contains(activeElement))) { event.preventDefault(); first.focus(); }
    };
    document.addEventListener('keydown', onKeyDown);
    return () => document.removeEventListener('keydown', onKeyDown);
  }, [mobile, open]);

  const closeSidebar = (restoreFocus: boolean) => { restoreFocusRef.current = restoreFocus; setOpen(false); };
  const openSidebar = () => { if (mobile) setOpen(true); };
  const toggleCollapsed = () => {
    const next = !collapsed;
    setCollapsed(next);
    document.documentElement.classList.toggle('sidebar-pre-collapsed', next);
    try { localStorage.setItem('hy2.sidebar', next ? 'collapsed' : 'expanded'); } catch { /* Storage can be disabled. */ }
  };
    ...
    <div className={`app${effectiveCollapsed ? ' sidebar-collapsed' : ''}${animationReady ? ' anim-ready' : ''}`} data-auth-status={authStatus}>
      <aside className={`sidebar${effectiveCollapsed ? ' collapsed' : ''}${mobile && open ? ' open' : ''}`} id="sidebar" ref={sidebarRef} inert={mobile && !open ? true : undefined}>
        ...
        <div className="sidebar-footer">
          {sidebarBottom ? <div className="sidebar-bottom">{sidebarBottom}</div> : null}
          {footerNavigation.map(item => <a key={item.key} href={item.href} className={`sidebar-link ${item.key === active ? 'active' : ''}`} aria-current={item.key === active ? 'page' : undefined} title={item.label} aria-label={item.label} onClick={() => closeSidebar(false)}><Icon name={item.icon}/><span>{item.label}</span></a>)}
      ...
      <div className="main" inert={mobile && open ? true : undefined}>
        ...
        </div><div className="topbar-right">{topbarExtra}{badge ? <span className="badge">{badge}</span> : null}</div></div></header>
