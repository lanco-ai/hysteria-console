  ...
  try {
    ...
    await page.route('**/api/plans', async route => {
      if (route.request().method() === 'PUT') {
        saveRequests += 1;
        const payload = route.request().postDataJSON();
        suggestionIds.push(payload.items.filter(item => ['完成项目提纲', '补充项目提纲', '补充项目提纲（已调整）'].includes(item.title)).map(item => item.id)[0] || '');
        if (rejectNextSave) {
    ...
    await expect(page.getByRole('button', { name: '添加并保存', exact: true })).toBeDisabled();
    initialRead.release();
    ...
    rejectNextSave = true;
    await page.getByRole('button', { name: '添加并保存', exact: true }).click();
    ...
    await expect(page.locator('.plans-save-status')).toHaveText('已保存');
    await expect(page.getByText('刷新期间编辑', { exact: true })).toHaveCount(0);
    assert.equal(saveRequests, 2, 'the two rejected local-add attempts must reach the save endpoint');
    saveRequests = 0;
    ...
    await expect(page.locator('.plans-save-status')).toHaveText('保存失败');
    await expect(page.getByRole('dialog', { name: '把目标整理成可选计划' })).toBeVisible();
    await expect(page.getByRole('dialog', { name: '把目标整理成可选计划' }).getByRole('alert')).toContainText('计划暂时无法读取或保存');
    ...
    await expect(page.locator('.plans-create').getByRole('button', { name: '保存中…' })).toBeDisabled();
    releaseSave();
    await expect(saveStatus).toHaveText('未保存');
    await expect(page.getByText('完成项目提纲', { exact: true })).toBeVisible();
    await expect(page.getByLabel('计划标题')).toHaveValue('保存期间编辑保留');
    ...
    const taskChecks = page.locator('.plans-task-check input');
    await taskChecks.first().check();
    ...
    await expect(taskChecks.first()).toBeChecked();
    await expect(taskChecks.nth(1)).toBeChecked();
    await page.getByRole('button', { name: '保存计划' }).click();
    await expect(saveStatus).toHaveText('已保存');
    ...
    await page.getByRole('button', { name: '保存选中建议' }).click();
    await expect(page.getByRole('dialog', { name: '把目标整理成可选计划' })).toHaveCount(0);
    assert.equal(saveRequests, savesBeforeNoopRetry, 're-applying an already committed suggestion must not send another save');
    const committedTasksAfterNoopRetry = savedItems.filter(item => item.id === committedSuggestionId);
    assert.equal(committedTasksAfterNoopRetry.length, 1, 'retrying after a lost acknowledgement must not duplicate the task');
    assert.equal(committedTasksAfterNoopRetry[0].id, committedTaskBeforeNoopRetry.id);
    ...
    await page.getByRole('button', { name: '保存选中建议' }).click();
    await expect(page.getByRole('dialog', { name: '把目标整理成可选计划' })).toHaveCount(0);
    await expect(page.locator('.plans-grid').getByText('补充项目提纲', { exact: true })).toHaveCount(0);
    await expect(page.locator('.plans-grid').getByText('补充项目提纲（已调整）', { exact: true })).toHaveCount(1);
    assert.equal(saveRequests, savesBeforeSemanticUpdate + 1, 'a semantic duplicate should update the existing task once');
    assert.equal(suggestionIds.at(-1), committedSuggestionId, 'a semantic update must retain the original task ID after a lost acknowledgement');
