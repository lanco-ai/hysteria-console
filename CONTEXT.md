...
A point-in-time snapshot of active hysteria2 sessions per user, published by hysteria's `/online` API and refreshed every cron tick (currently 5 s) into `online.json`. Treated as synonymous with "active devices" because each device typically holds one session. Used by `auth_backend.py` to enforce `max_devices` at *login* time only — sessions already established are never killed for exceeding `max_devices`, only new logins are rejected. Note: this snapshot can flicker between ticks; it is not authoritative for billing.
...
> **Operator:** "No, **online** is a 5s snapshot. It can flicker. We only enforce `max_devices` at login, not in real time."
