...
def _line_radar_protocol_totals(*, now, hours=24):
    ...
    for hk in _line_radar_hour_keys(now, hours=hours):
        bucket = hourly.get(hk) or {}
        for proto in totals:
            totals[proto] += _entry_total(bucket.get(proto))
    scaled = {proto: int(total * DISPLAY_MULTIPLIER) for proto, total in totals.items()}
...
def build_line_radar(*, now=None):
    ...
    users = load_json(USERS_FILE, {})
    online = load_json(ONLINE_FILE, {})
    totals, total_bytes = _line_radar_protocol_totals(now=now)
    service_states = {
        key: probe_systemd(meta['unit'])
...
def render_line_radar(now=None):
    ...
    for row in radar['rows']:
        ...
        traffic = fmt_bytes(row['bytes']) if row['key'] != 'tuic' else '暂不可计量'
    ...
    return (
        ...
        f'<div class="small">近 {radar["window_hours"]} 小时协议占比 · 总量 {fmt_bytes(radar["total_bytes"])}</div></div>'
