...
def _incident_response(payload, *, attachment=False):
    model = AdminIncidentResponse.model_validate(payload)
    headers = None
    if attachment:
...
def create_app(services, *, max_requests=32):
    ...
    @app.api_route('/api/v1/admin/incidents', methods=['GET', 'HEAD'])
    async def admin_incidents(request: Request):
        ...
        if isinstance(payload, JSONResponse):
            return payload
        return _incident_response(payload)

    @app.api_route('/api/v1/admin/incidents/evidence', methods=['GET', 'HEAD'])
    async def admin_incident_evidence(request: Request):
        try:
            payload = await dispatch(services.read_admin_incidents, request)
        except (LoginRequired, UserAccessDenied, StateUnavailable) as exc:
            return _read_error_response(exc)
