...
- **用户面板** —— `http://<server>/panel/<user>?token=<token>` —— 单用户的流量与设备统计，含配额重置倒计时、近 30 天用量趋势、订阅/面板链接一键复制、多 profile 订阅导出，并每 10 秒自动刷新用量（标签页隐藏时暂停）。
...
- **Codex 额度** —— `http://<server>/admin/codex` —— 读取本机 Codex app-server 的账户额度，显示当前余量、重置倒计时与日/周/月/年趋势。采集器是每 3 分钟启动一次的 one-shot 进程，历史按 3 分钟、15 分钟、2 小时分层压缩；若上游没有提供某个额度窗口，面板会明确显示“未提供”而不是推算旧值。
