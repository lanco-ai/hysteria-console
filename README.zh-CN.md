...
- **管理后台** —— `http://<server>/admin` —— 在 `/login` 登录。管理员密码**不是**通过首次访问的表单设置的。首次部署时请在 `subscription_meta.json` 写入明文 `admin_pass`，下次启动会自动迁移为加盐的 PBKDF2 `admin_pass_hash` 并删除明文；若未配置任何密码，则会随机生成一个写入哈希（因此请自行设置 `admin_pass` 或事后轮换）。随时可在 `/admin/settings` 轮换密码——轮换会注销其它所有会话，同时保持当前设备登录。
...
- **健康状态** —— 6 张状态卡之外，提供「发送测试告警」按钮，验证 `alerts.json` 的 Telegram / webhook 是否配通。webhook URL 由运营者自行配置（视为管理员级信任），同步发起请求，不做 URL 白名单限制。
